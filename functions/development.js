...
exports.addTestObservations = async function addTestObservations(req, res) {
...
exports.calcMaxTemperatures = async (req, res) => {
  ...
  try {
    const observationsByDate = await admin
      .firestore()
      .collection('observations')
      .where('utctime', '>', +from / 1000)
      .where('utctime', '<', +until / 1000)
      .get()
      .then(querySnapshot => {
        return querySnapshot.docs.reduce((acc, documentSnapshot) => {
          const data = documentSnapshot.data();
          const dateString = new Date(data.utctime * 1000).toISOString().slice(0, 10);
    ...
    const promises = Object.entries(observationsByDate).map(([dateString, data]) => {
      const maxTemperature = Math.max(...data.map(d => d.tempout));
      const newDoc = { date: new Date(dateString), maxTemperature };
      return admin
        .firestore()
